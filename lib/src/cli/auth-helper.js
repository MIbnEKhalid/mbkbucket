...
export const getStoredToken = () => readConfig().token || null;
export const getStoredServerUrl = () => readConfig().server_url || null;
...
export async function deviceFlowLogin({ server_url, profile_key }) {
  ...
  const { verification_url = '', user_code = '', device_code, expires_in = 900, interval = 5, profile } = deviceResp;
