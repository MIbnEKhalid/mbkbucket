...
const REQUIRED_BUCKET_FIELDS = ['BUCKET_NAME', 'ACCESS_KEY_ID', 'SECRET_ACCESS_KEY', 'ENDPOINT'];
...
export function parseAndValidateMbkbucketVar(rawValue) {
  ...
  const defaults = { p_view_inline: true, publiView_enabled: false };
  ...
  const errors = [];
  if (typeof parsed.p_view_inline !== 'boolean') errors.push('mbkbucketVar.p_view_inline must be a boolean if provided');
  if (typeof parsed.publiView_enabled !== 'boolean') errors.push('mbkbucketVar.publiView_enabled must be a boolean if provided');
...
export function parseAndValidateBucketConnection(rawValue) {
  ...
  for (const [bucketName, config] of Object.entries(parsed)) {
    ...
    const missingFields = REQUIRED_BUCKET_FIELDS.filter(field => !config[field]);
