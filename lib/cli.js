...
import { readConfig, clearConfig, updateConfig, writeConfig, isLoggedIn, deviceFlowLogin, CONFIG_FILE, CONFIG_DIR } from './src/cli/auth-helper.js';
...
const cmdArgs = positionalArgs.slice(1);
...
async function cmdLogin() {
  ...
  const server_url = findFlag('server') || storedCfg.server_url;

  if (!server_url || typeof server_url !== 'string') {
    console.error('❌ --server <url> is required for login.\n   Example: mbkbucket login --server https://myapp.example.com\n\n💡 Tip: Set it once with:\n   mbkbucket config set server_url https://your-server.com\n   Then just run: mbkbucket login');
    process.exit(1);
  }

  const profile_key = findFlag('profile-key') || storedCfg.profile_key;
  updateConfig({ server_url });

  try {
    const result = await deviceFlowLogin({
      server_url,
      profile_key: typeof profile_key === 'string' ? profile_key : undefined,
    });
    ...
    if (msg.includes('profile_key') || msg.includes('profile_id')) {
      ...
      console.error(`\n💡 Could not reach server "${server_url}". Verify it is running and CLI_AUTH_ENABLED=true.`);
    }
    process.exit(1);
  }
}
...
const DEMO_CONFIG = {
  ...
  token_prefix: 'mbk_demo',
  username: 'demo-user',
  server_url: 'https://your-app.example.com',
  profile: { name: 'Default CLI Profile', scope: 'read-write', allowed_apps: ['mbkbucket'] },
  logged_in_at: nowIso(),
};

const VALID_CONFIG_KEYS = ['token', 'token_prefix', 'username', 'server_url', 'profile', 'profile_key', 'logged_in_at'];
