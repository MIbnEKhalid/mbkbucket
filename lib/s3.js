...
export function getAvailableBucketNames() {
  return Object.keys(allBucketConfigs || {});
}
...
export function resolveBucketName(bucketName) {
  ...
  if (!allBucketConfigs[candidate]) {
    throw new Error(`Bucket '${candidate}' not found in BucketConnection. Available buckets: ${Object.keys(allBucketConfigs).join(', ')}`);
  }
...
function isRootModeApp(appName) {
  return String(appName || '').trim().toLowerCase() === 'portal';
}
