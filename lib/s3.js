...
function getDefaultBucketName() {
  ...
  const buckets = Object.keys(allBucketConfigs);
  if (buckets.length > 0) {
    return buckets[0];
  }
...
export function getBucketConfig(bucketName) {
  // Resolve default bucket name if not provided
  if (!bucketName) {
    bucketName = getDefaultBucketName();
    if (!bucketName) {
      throw new Error('No bucket name provided and no default bucket configured. Please set mbkautheVar.bucket or configure BucketConnection.');
    }
  }
  ...
  if (!allBucketConfigs || Object.keys(allBucketConfigs).length === 0) {
  ...
  if (!cfg) {
    throw new Error(`Bucket '${bucketName}' not found in BucketConnection. Available buckets: ${Object.keys(allBucketConfigs).join(', ')}`);
  }
 ...
 */
export async function createMultipartUpload(key, contentType = 'application/octet-stream', metadata = {}) {
...
export async function listIncompleteMultipartUploads(prefix = '', bucketName) {
  try {
    const effectivePrefix = prefix ? ensurePrefix(prefix) : getAppName();
...
export async function cleanupIncompleteMultipartUploads(olderThanDays = 7, prefix = '', bucketName) {
  try {
    ...
    if (!uploads.length) {
      console.log('[mbkbucket] No incomplete multipart uploads found');
    ...
    if (!uploadsToAbort.length) {
      console.log(`[mbkbucket] No incomplete uploads older than ${olderThanDays} days`);
      return { abortedCount: 0, uploads: [] };
    }
    
    console.log(`[mbkbucket] Found ${uploadsToAbort.length} incomplete uploads to clean up`);
    ...
    for (const upload of uploadsToAbort) {
      try {
        ...
        console.log(`[mbkbucket] Aborted incomplete upload: ${upload.Key} (initiated: ${upload.Initiated})`);
      } catch (err) {
