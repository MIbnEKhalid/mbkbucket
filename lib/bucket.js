...
router.get(['/mbkbucket/info', "/mbkbucket/i"], validateSessionAndRole('SuperAdmin'), async (req, res) => {
  ...
  renderPage(req, res, "info.handlebars", false, { page: "MBK Bucket Info", appVersion, latestVersion, APP_NAME: mbkautheVar?.APP_NAME });
});
  ...
  try {
    ...
    if (useOptimizedListing) {
      ...
      do {
        ...
        if (result.Contents) {
          files = files.concat(result.Contents);
        }
    ...
    if (useOptimizedListing) {
       res.json({
         ...
         search: search,
         // In optimized mode, we don't support random access "Page X". We support "Next".
         currentPage: 1, 
         totalPages: nextContinuationToken ? 9999 : 1, 
